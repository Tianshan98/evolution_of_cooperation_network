...
class NetworkSimulation(object):
    ...
    def run_trial(self, trial_id=0):
        ...
        #####Self Code
        all_agents = [self.env.G.node[i]['agent'] for i in self.env.G.nodes()]

        pool = Pool(500)
        for i in range(1, 500):
            pool.map(target = self.env.process, args = agents)
        pool.close()

        #for i in self.env.G.nodes():
            #self.env.process(self.env.G.node[i]['agent'])
        #self.env.run()
    ...
    def setup_network_agents(self):
        ...
        for i in self.env.G.nodes():
            ...
            #print('self.env.G.node type:', type(self.env.G.node[i]['agent']))
